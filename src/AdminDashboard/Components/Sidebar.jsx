...
const Sidebar = ({
  ...
  useEffect(() => {
    ...
    const fetchSprints = async () => {
      try {
        const data = await getSprint(selectedId); // ✅ only selectedId
        setSprints(data); // ✅ data is already array
      } catch (error) {
  ...
  const filteredSprints = selectedId
    ? sprints.filter((s) => String(s.project_id) === String(selectedId))
    : [];
            ...
            {!selectedId ? (
              ...
              filteredSprints.map((sprint) => (
                <div key={sprint.id} className="ml-4">
                  <div className="flex items-center px-3 py-1 rounded-lg justify-between hover:bg-blue-100">
                    <div className="flex gap-3 items-center text-sm cursor-pointer">
                      <Timer size={15} /> {sprint.name}
                    </div>

                    <button
                      className="text-red-600 hover:scale-110 cursor-pointer px-2 py-1"
                      onClick={(e) => {
                        e.stopPropagation();
